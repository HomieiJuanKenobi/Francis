...
def Attaque_passeur(state):
    return aller_courrir(state,state.ball_position()+state.ball_vitesse()*5)+shoot(state, state.ally_position()+state.ally_vitesse())
...
def Attaque_campeur_proche_2v2(state):
    if (peut_shooter(state)):
...
def Attaque_dribbleur_2v2(state):
    if (peut_shooter(state)):
        return aller_courrir_marcher(state,state.ball_position()+state.ball_vitesse()*5)+mini_shoot(state, state.but_adv[state.key[0]-1])
    else:
        return aller_courrir_marcher(state,state.ball_position()+state.ball_vitesse()*5)
    
def Defense_gardien_2v2(state): 
    if (peut_shooter(state)):
        ...
        if state.je_suis_dans_mon_camp():
            if (distance(state.ball_position(),state.my_position())<10):
                return aller_courrir_marcher(state,state.ball_position())
            else:
...
def aller_camper_proche(state,p):
    ...
    if (state.key[0]==2):
        if (p.x>(3*settings.GAME_WIDTH)/4) :
            ...
            v2=p
    return aller_courrir_marcher(state,v2)
...
def aller_gardien(state,p):
    if (state.key[0]==1):
        if (state.ball_position().x+state.ball_vitesse().x>=(settings.GAME_WIDTH)/3) :
            ...
            return aller_courrir_marcher(state,v2+v3)       
        else:
            return aller_courrir_marcher(state,p) 
    if (state.key[0]==2):
